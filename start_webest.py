...
def list_python_candidates():
    ...
    if python3_from_path:
        candidates.append([python3_from_path])
    ...
    if os.name == "nt":
        ...
        try:
            result = subprocess.run(
                ["bash", "-lc", "command -v -a python python3 2>/dev/null || true"],
                capture_output=True,
                text=True,
                timeout=5,
                check=False,
...
def can_import_backend(python_cmd, server_dir):
    if not can_import_module(python_cmd, "waitress", server_dir):
        return False
    return can_import_module(python_cmd, "domain_admin.main", server_dir)

def ensure_backend_ready(python_cmd, server_dir):
    if can_import_backend(python_cmd, server_dir):
        return True
    try:
        print("[*] 检测到后端依赖缺失，正在自动安装 requirements/production.txt + waitress ...")
        subprocess.run(
        ...
        subprocess.run(
            python_cmd + ["-m", "pip", "install", "-r", "requirements/production.txt"],
            cwd=server_dir,
            timeout=600,
            check=False,
        )
        ...
        return False
    return can_import_backend(python_cmd, server_dir)

def choose_backend_python(server_dir):
    """选择可用的 Python 解释器，确保能导入 waitress"""
    for cmd in list_python_candidates():
        if can_import_backend(cmd, server_dir):
            return cmd

    for cmd in list_python_candidates():
        if ensure_backend_ready(cmd, server_dir):
            return cmd
...
def main():
    ...
    print("[👉] 默认账号: webest666   密码: 8D673F2a!")
